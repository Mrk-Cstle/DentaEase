...
<div class="flex flex-row h-full gap-5">
    <div class="rounded-md flex flex-col w-[30%] bg-white shadow p-4">
        ...
        @else
        <div class="aspect-square bg-cover bg-no-repeat bg-center rounded-t-md bg-[url({{ asset('DentaEase/public/storage/profile_pictures/' . Auth::user()->profile_image) }})]">
        @endif
        </div>
    ...
    <div class="flex flex-col basis-[70%] gap-5">
        <div class="rounded-md bg-white shadow flex flex-row gap-3 p-5">
            <div class="basis-[50%] border rounded p-4 flex items-center justify-center">
                <a href="{{ asset('DentaEase/public/storage/qr_codes/' . Auth::user()->qr_code) }}" download="qr_code.svg">
                <img src="{{ asset('DentaEase/public/storage/qr_codes/' . Auth::user()->qr_code) }}" alt="QR Code" class="w-32 h-32 hover:opacity-80" />
                <p class="text-blue-600 text-sm mt-2 text-center hover:underline">Download QR Code</p>
                </a>
            </div>
            <div class="basis-[50%] border rounded p-4 flex flex-col justify-between">
                <div class="flex justify-between items-center mb-4">
                    <p class="font-semibold">Face Recognition</p>
                    <button id="removeFaceToken" class="bg-red-600 hover:bg-red-700 px-3 py-1 text-white rounded">Remove</button>
                </div>
                @if(Auth::user()->face_token !== null && Auth::user()->face_token !== "")
                    <button id="capturemodal" class="px-4 py-2 bg-blue-300 text-white rounded" disabled>Capture & Register</button>
                @else
                    <button id="capturemodal" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded">Capture & Register</button>
                @endif
            </div>
        </div>
